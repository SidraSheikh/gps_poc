...
async function initializeMap() {
  try {
    ...
    console.error(
      "CRITICAL ERROR: Could not fetch initial point. Is the backend server running?",
      error
    );
    alert(
      "Could not connect to the server. Please ensure the backend (server.js) is running and refresh the page."
    );
  }
}
...
function runMapLogic(map) {
  ...
  const ZOOM_THRESHOLDS = [
    { zoom: 10, sampleRate: 50 },
    { zoom: 12, sampleRate: 10 },
    { zoom: 14, sampleRate: 5 },
    { zoom: 16, sampleRate: 2 },
  ...
  function renderVisiblePoints() {
    ...
    console.log(
      `Rendered ${renderedCount} points from ${totalCachedInView} in view.`
    );
  }
  ...
  async function onPointClick(e) {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/point-details/${clickedPointId}`
      );
      if (!response.ok)
      ...
      e.target
        .bindPopup(
          `
                ...
                <b>Frame Time (float):</b> ${pointDetails.frame_time.toFixed(
                  3
                )}<br>
  ...
  const fetchNewPoints = async () => {
    ...
    const queryParams = new URLSearchParams({
      minLng: extendedBounds.getWest(),
      minLat: extendedBounds.getSouth(),
      maxLng: extendedBounds.getEast(),
      maxLat: extendedBounds.getNorth()
    }).toString();

    try {
      const response = await fetch(
        `${API_BASE_URL}/api/map-points?${queryParams}`
      );
      if (!response.ok)
        throw new Error(`HTTP error! status: ${response.status}`);
      ...
      console.log(
        `Fetched ${newPoints.length} points, added ${pointsAdded} new. Total cached: ${cachedPoints.size}`
      );
      renderVisiblePoints();
    } catch (error) {
...
function setupOffsetCalculation(map) {
  ...
  calculateBtn.addEventListener("click", () => {
    ...
    const viewportHeight = map.distance(
      { lat: northEast.lat, lng: centerPoint.lng },
      { lat: southWest.lat, lng: centerPoint.lng }
    );
    const viewportWidth = map.distance(
      { lat: centerPoint.lat, lng: northEast.lng },
      { lat: centerPoint.lat, lng: southWest.lng }
    );
    ...
    L.marker([northPoint.lat, northPoint.lng], { icon: redMarkerIcon })
      .addTo(offsetMarkersLayer)
      .bindPopup("<b>North Offset Point</b>");
    L.marker([eastPoint.lat, eastPoint.lng], { icon: redMarkerIcon })
      .addTo(offsetMarkersLayer)
      .bindPopup("<b>East Offset Point</b>");
    L.marker([southPoint.lat, southPoint.lng], { icon: redMarkerIcon })
      .addTo(offsetMarkersLayer)
      .bindPopup("<b>South Offset Point</b>");
    L.marker([westPoint.lat, westPoint.lng], { icon: redMarkerIcon })
      .addTo(offsetMarkersLayer)
      .bindPopup("<b>West Offset Point</b>");
  });
}
